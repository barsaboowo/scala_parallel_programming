...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = points match {
    case Nil => means.map((_, Nil)).toMap
  ...
  def sdlt(eta: Double, tuple: (Point, Point)): Boolean = tuple._1.squareDistance(tuple._2) <= eta

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall(sdlt(eta, _))
  }
