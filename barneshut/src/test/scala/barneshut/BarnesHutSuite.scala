...
import FloatOps._
  ...
  test("insert when size > minsize"){

    val b = new Body(123f, 18f, 26f, 0f, 0f)
    val b2 = new Body(125f, 19f, 27f, 0f, 0f)
    val nw = Leaf(4f, 4f, 8f, Seq(b))
    val q = nw.insert(b2)
    q match {
      case Fork(nw, ne, sw, se) =>
      case _ => fail("wrong type : $q")
    }
  }
  ...
  test("values of a fork which is empty"){
    val f = new Fork(
      new Empty(51f, 46.3f, 5f),
      new Empty(51f, 46.3f, 5f),
      new Empty(51f, 46.3f, 5f),
      new Empty(51f, 46.3f, 5f)
    )
  ...
  test("computeSectorMatrix' should correctly add points to buckets given 7 points within a boundary of size 96::2") {
    val body1 = new Body(5, 25, 47, 0.1f, 0.1f)
    val body2 = new Body(5, 1, 47, 0.1f, 0.1f)
    val body3 = new Body(5, 96, 96, 0.1f, 0.1f)
    val body4 = new Body(5, -1, 98, 0.1f, 0.1f)
    val body5 = new Body(5, 98, -1, 0.1f, 0.1f)
    ...
    sm += body4
    sm += body5
    val res = sm(2, 3).size == 1 && sm(2, 3).find(_ == body1).isDefined
    assert(res, s"Body not found in the right sector")
  }
